...
data_folder_path = "/Volumes/Samsung USB/data/2025-04-08"
number_of_images = 60
shuffle = True

#generate Nest Images folder
def make_nest_images_dir():
    if not os.path.isdir(f'{data_folder_path}/Nest Images'):
        print("Need to make nest folder!")
        try:
            os.makedirs(f'{data_folder_path}/Nest Images')
            return 0, f'{data_folder_path}/Nest Images'
            ...
            try:
                nest_folder_path = f'{data_folder_path}/Nest Images'
...
def generate_nest_image(todays_folder_path, today, number_of_images, hostname, shuffle=True):
    ...
    print("Image written!")
    print(f"Composite image written using {total_frames} images!")
    return total_frames
...
def main():
    ...
    #parser.add_argument('-p', '--data_folder_path', type=str, default=data_folder_path, help='a path to the folder you want to collect data in. Default path is: /mnt/bumblebox/data/')
    #parser.add_argument('-i', '--number_of_images', type=int, default=number_of_images, help='the number of images you want to use to create your composite nest labelling image from todays images. The more you use, the longer this takes!')
    ...
    if sys.stdout.isatty():
        ...
        if yes_or_no == "yes":
            ...
            todays_folder_path = data_folder_path
    ...
    total_frames = generate_nest_image(todays_folder_path, today, number_of_images, hostname, shuffle=True)
